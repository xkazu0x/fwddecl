#ifndef KRUEGER_OS_AUDIO_LINUX_C
#define KRUEGER_OS_AUDIO_LINUX_C

////////////////////////
// NOTE: Linux Functions

internal void *
_linux_alsa_thread_fn(void *param) {
  for (;;) {
    ...
    if (write_res < 0) {
      ...
      _os_audio_desc.callback((s16 *)_lnx_audio_state.buffer, _lnx_audio_state.num_frames, _os_audio_desc.user_data);
    }
  }
  return(0);
}
...
internal void
os_audio_init(void) {
  u32 rate = _os_audio_desc.sample_rate;
  int dir = 0;
  ...
  snd_pcm_hw_params_set_channels(_lnx_audio_state.device, hw_params, _os_audio_desc.num_channels);
  ...
  _os_audio_desc.sample_rate = rate;
  _lnx_audio_arena = arena_alloc();
  _lnx_audio_state.num_frames = 2048;
  u32 bytes_per_frame = _os_audio_desc.num_channels*sizeof(s16);
  ...
  pthread_t thread;
  pthread_create(&thread, 0, _linux_alsa_thread_fn, 0);
}
...
#endif // KRUEGER_OS_AUDIO_LINUX_C
