...
#undef WGL_PROC
  ...
  _ogl_win32_ctx = wglCreateContextAttribsARB(hdc, hglrc, ctx_attribs);
...
internal void
ogl_window_equip(Os_Handle handle) {
...
internal void
ogl_window_select(Os_Handle handle) {
  if (os_handle_is_valid(handle)) {
    _Win32_Window *window = _win32_window_from_handle(handle);
    wglMakeCurrent(window->hdc, _ogl_win32_ctx);
  }
}

internal void
ogl_window_swap(Os_Handle handle) {
  if (os_handle_is_valid(handle)) {
    _Win32_Window *window = _win32_window_from_handle(handle);
